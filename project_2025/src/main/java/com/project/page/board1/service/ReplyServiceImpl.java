...
public class ReplyServiceImpl implements ReplyService {
	...
	private void initChildReply(Reply reply) throws ReplyServiceException {
		Reply parentReply = replydao.selectReplyById(reply.getParentId());
		
		if(parentReply == null) {
			logger.error("부노 댓글을 찾을 수 없습니다. parentId : {}", reply.getParentId());
		...
		reply.setReplyClass(parentReply.getReplyClass() + 1);
	}

	...
	<main id="article-main" class="container">
	    ...
	    <section class="row g-5">
		    <form class="row g-3 comment-form" id="comment-form">
			    <input type="hidden" id="replyClass" value="">
        ...
        {{#each replies}}
            <li class="list-group-item" style="margin-left:{{calcIndent this}}px">
                <div>
                    ...
                    {{#isRoot replyClass}}
                        <button class="btn btn-link btn-sm reply-btn" data-reply-id="{{replyId}}">답글</button>
                        <div class="reply-form mt-2" id="replyForm-{{replyId}}" style="display:none;">
                ...
                {{#if childReplies}}
                    <ul class="list-group mt-2">
                        {{> replyPartial replies=childReplies}}
                    </ul>
                {{/if}}
            </li>
        {{/each}}
...
$(function() {
    ...
    Handlebars.registerHelper('calcIndent', function(reply) {
        return (reply.replyClass && reply.replyClass != 0) ? 20 : 0;
    });
    Handlebars.registerHelper('isRoot', function(replyClass, options) {
        return (!replyClass || replyClass == 0) ? options.fn(this) : options.inverse(this);
    });
    ...
    function submitReply(content, replyClass, done) {
        const payload = {
            replyContent: content,
            replyClass: replyClass || 0,
            userId: "USER"
        };
    ...
    $('#comment-form').submit(function(e) {
        ...
        const replyClass = $('#replyClass').val() || 0;

        const $btn = $(this).find('button[type="submit"]');
        $btn.prop('disabled', true);

        submitReply(content, replyClass, () => {
            $('#replyContent').val('');
            $('#replyClass').val('');
